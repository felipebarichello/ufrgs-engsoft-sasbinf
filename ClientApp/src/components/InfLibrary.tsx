...
export default function INFLibrary({
	...
	async function handleBookPress() {
		if (selected === null) {
			alert(
				'Nenhuma sala está selecionada. Clique na sala para selecioná-la.'
			);
			return;
		}
		...
		const bookRequest = {
			...
			startTime: `${new Date(
				filtersState.day + ' ' + filtersState.startTime
			)
				.toTimeString()
				.slice(0, 8)}.000`,
		...
		try {
			...
			const newAvailableState =
				await triggerAvailableRoomsQuery(filtersState).unwrap();
...
function StandaloneTables() {
	return (
		<div className="d-flex flex-column">
			<VerticalSpacer
				size={`${((1280 + 100) * 0.25 * 0.4 * 36) / 84}px`}
			/>
